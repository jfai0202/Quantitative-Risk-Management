...
output_path = os.path.join(base_path, "myout_10_4.csv")
...
weights = np.round(weights, 10)

weights[np.abs(weights - 0.1) < 1e-6] = 0.1
weights[np.abs(weights - 0.2) < 1e-6] = 0.2
weights[np.abs(weights - 0.5) < 1e-6] = 0.5
